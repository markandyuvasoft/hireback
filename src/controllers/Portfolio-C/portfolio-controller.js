...
export const createPortfolio = async (req, res) => {
  try {
    ...
    if (req.files && req.files.portfolioImage) {
      const portfolioImageUpload = await cloudinary.uploader.upload(
        req.files.portfolioImage[0].path,
        {
          folder: "cover_blogs",
          overwrite: false,
        }
      );
      portfolioImage = portfolioImageUpload;
    }
  ...
  try {
    ...
    if (req.files && req.files.portfolioImage) {
      const portfolioImageUpload = await cloudinary.uploader.upload(
        ...
        {
          folder: "cover_blogs",
          public_id: `profile_${portfolioId}`,
          overwrite: true,
        }
      );
      portfolioImage = portfolioImageUpload.secure_url;
    } else if (req.body.oldImage) {
      portfolioImage = req.body.oldImage;
    } else {
      const existingPortfolio = await Portfolio.findById(portfolioId);
      if (existingPortfolio && existingPortfolio.portfolioImage) {
        portfolioImage = existingPortfolio.portfolioImage;
      }
    }
